...
private static Graph getGraph(String[] forms, String[] pos, int[] heads, int[] labelids, String[] labels) throws Exception {
        //ArrayList<TaggedWord> words = tree.taggedYield();
        //GrammaticalStructure gs = gsf.newGrammaticalStructure(tree);
        Graph g = new Graph(forms, pos, labelids, labels);
        for(int i=1; i<forms.length; i++)
        {
            g.addEdge(i-1, heads[i]-1, labels[labelids[i]]);
        }
        g.setRoot(g.nodes.get(0).label);
//        for (TypedDependency td : tdl) {
//            g.addEdge(td.gov().index() - 1, td.dep().index() - 1, td.reln().toString());
...
//    }
    ...
    public static void writeImage(String[] forms, String[] pos, int[] heads, int[] labelids, String[] labels, String outFile, int scale) throws Exception {
        Graph g = getGraph(forms, pos, heads, labelids, labels);
        BufferedImage image = createTextImage(g, scale);
        ImageIO.write(image, "png", new File(outFile));
    }
